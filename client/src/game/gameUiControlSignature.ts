...
export const getGameUiControlLayoutSignature = (
  ...
  controlPanel: {
    ...
    constructorProductionJobs: state.constructorProductionJobs.map((job) => `${job.id}:${job.constructorEquipmentGroupId}:${job.queueType}:${job.productItemModelId}:${job.remainingTime}:${job.totalTime}:${Number(job.running)}`).join(","),
