...
export const getGameUiControlLayoutSignature = (
  ...
  exchange: {
    visible: state.exchangeState !== null,
    openSelect: state.openExchangeSelect,
    receiverContainerId: state.exchangeState?.selfReceiverContainerEquipmentGroupId ?? null,
    sourceContainerId: state.exchangeState?.selfSourceContainerEquipmentGroupId ?? null,
    receiverObjectId: state.selectedExchangeReceiverObjectId,
    sourceObjectId: state.selectedExchangeSourceObjectId,
    selectedSourceItems: (state.selectedExchangeSourceItemGroupIds ?? []).join(","),
    selfConfirmed: state.exchangeState?.selfConfirmed ?? false,
    otherConfirmed: state.exchangeState?.otherConfirmed ?? false,
    notEnoughSpace: state.exchangeState?.notEnoughSpace ?? false,
    amountDialogOpen: state.controlPanelContainerTransferDialogOpen,
    amountMax: state.controlPanelContainerTransferMaxAmount,
    amountText: state.controlPanelFuelDrainAmountText,
    amountFocused: state.controlPanelFuelDrainAmountFocused,
    selfQueue: state.exchangeState?.selfQueue.map((row) => `${row.taskItemGroupId}:${row.count}:${row.progress}:${Number(row.isReady)}`).join(",") ?? "",
    otherQueue: state.exchangeState?.otherQueue.map((row) => `${row.taskItemGroupId}:${row.count}:${row.progress}:${Number(row.isReady)}`).join(",") ?? "",
  },
  settings: {
  ...
  controlPanel: {
    ...
    selectedUsageLeftObjectId: state.selectedControlPanelUsageLeftObjectId,
    selectedUsageRightObjectId: state.selectedControlPanelUsageRightObjectId,
    selectedConstructorMaterialObjectId: state.selectedControlPanelConstructorMaterialObjectId,
    selectedConstructorProductObjectId: state.selectedControlPanelConstructorProductObjectId,
    ...
    selectedConstructorProductContainerGroupId: state.selectedControlPanelConstructorProductContainerGroupId,
    ...
    constructorProduceDialogOpen: state.controlPanelConstructorProduceDialogOpen,
    itemDeconstructionDialogOpen: state.controlPanelItemDeconstructionDialogOpen,
    ...
    constructorProduceMaxAmount: state.controlPanelConstructorProduceMaxAmount,
    itemDeconstructionMaxAmount: state.controlPanelItemDeconstructionMaxAmount,
