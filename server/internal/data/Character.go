...
// Character хранит данные одного персонажа игрового мира.
...
// Characters хранит персонажей и быстрые индексы для поиска по связанным объектам.
...
// NewCharacters создаёт пустое хранилище персонажей с подготовленными индексами.
...
// Add добавляет нового персонажа, назначает новый ID и время создания.
...
// Get возвращает персонажа по ID.
...
// Delete удаляет персонажа и все его быстрые индексы.
...
// GetByAccountID возвращает персонажей указанного аккаунта в порядке возрастания ID.
...
// LoadFromFile загружает персонажей из JSON-файла и пересобирает быстрые индексы.
...
// SaveToFile сохраняет персонажей в JSON-файл без вспомогательных индексов.
...
// ensureItems подготавливает основную map персонажей.
...
// validateRequiredFields проверяет обязательные поля персонажа.
...
// addIndexes добавляет персонажа во все быстрые индексы.
...
// deleteIndexes удаляет персонажа из всех быстрых индексов.
