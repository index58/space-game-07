...
// CosmicObject хранит данные одного космического объекта игрового мира.
...
// CosmicObjects хранит космические объекты и быстрые индексы по связанным объектам.
...
// NewCosmicObjects создаёт пустое хранилище космических объектов с подготовленными индексами.
...
// Add добавляет новый космический объект и назначает новый ID.
...
// Get возвращает космический объект по ID.
...
// Delete удаляет космический объект и все его быстрые индексы.
...
// GetByCosmicObjectModelID возвращает объекты указанной модели в порядке возрастания ID.
...
// GetByOwnerCharacterID возвращает объекты указанного владельца-персонажа в порядке возрастания ID.
...
// LoadFromFile загружает космические объекты из JSON-файла и пересобирает быстрые индексы.
...
// SaveToFile сохраняет космические объекты в JSON-файл без вспомогательных индексов.
...
// ensureItems подготавливает основную map космических объектов.
...
// validateRequiredFields проверяет обязательные поля космического объекта.
...
// addIndexes добавляет космический объект во все быстрые индексы.
...
// deleteIndexes удаляет космический объект из всех быстрых индексов.
...
// addCosmicObjectIndex добавляет объект в неуникальный индекс.
...
// deleteCosmicObjectIndex удаляет объект из неуникального индекса.
...
// sortedCosmicObjects возвращает объекты индекса в стабильном порядке ID.
