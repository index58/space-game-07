...
func testWorldData(t *testing.T) world.Data {
	...
	itemProductionType, err := taskTypes.Add(&data.TaskType{TitleRu: "Производство предметов", TitleEn: "Item production", Acronym: "ItemProduction"})
	if err != nil {
		t.Fatal(err)
	}
	objectProductionType, err := taskTypes.Add(&data.TaskType{TitleRu: "Производство объектов", TitleEn: "Object production", Acronym: "ObjectProduction"})
	if err != nil {
		t.Fatal(err)
	}
